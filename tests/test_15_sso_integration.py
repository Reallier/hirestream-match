...
# 根据 TEST_ENV 环境变量选择测试目标
_TEST_ENV = os.getenv("TEST_ENV", "test")
...
class TestInfrastructure:
    ...
    def test_jwt_001_secret_consistency(self):
        ...
        assert data.get("user", {}).get("id") == TEST_USER_ID, f"用户 ID 解析错误: {data}"


# ============= SSO 认证流程测试 =============

@pytest.mark.skip(reason="需要实际登录凭证，在 CI/CD 中跳过")
class TestSSOFlow:
    """SSO 单点登录流程测试 - 需要实际登录凭证"""
...
class TestJWTCompatibility:
    ...
    @pytest.mark.skip(reason="需要实际登录凭证")
    ...
    def test_jwt_format_003_unified_integer_user_id(self):
        ...
        data = response.json()
        assert data.get("success") is True, f"用户验证失败: {data}"
        assert data.get("user", {}).get("id") == TEST_USER_ID, f"用户 ID 解析失败: 期望 {TEST_USER_ID}, 实际 {data}"
